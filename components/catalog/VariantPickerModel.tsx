...
export default function VariantPickerModal({ open, onClose, product }: Props) {
  const router = useRouter();
  const { user } = useAuthStore();
  const cart = useCartStore();
  ...
  // true buy-now: initiate checkout session from product
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            {selectedVariant ? (
              <div className="rounded-xl border bg-gray-50 p-4">
                ...
                {inCart ? (
                  <div className="mt-2 text-xs text-gray-600">
                    This variant is already in your cart. “Add to cart” will update its quantity.
                  </div>
                ) : null}
              </div>
            ) : null}
